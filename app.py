...
def main():
    ...
    with tab3:
        ...
        if process_btn:
            with st.spinner("🔄 Processing clusters..."):
                ...
                cluster_metrics['Avg Distance (km)'] = 0
                cluster_metrics['Transport Cost (INR)'] = 0
                ...
                for cluster_id in range(n_clusters):
                    ...
                    if len(cluster_data) > 1:
                        ...
                        cluster_metrics.loc[cluster_metrics['Cluster'] == cluster_id, 'Avg Distance (km)'] = avg_dist
                        cluster_metrics.loc[cluster_metrics['Cluster'] == cluster_id, 'Transport Cost (INR)'] = \
                            avg_dist * assumptions["mobile_unit_cost_per_km"]
    ...
    with tab5:
        ...
        ops_display['Processing Time (hrs)'] = (ops_display['Total Stubble (tons)'] * assumptions['processing_time_per_ton']).round(1)
        ...
        # IMPACT PANEL
        st.markdown("## 🌍 4. Impact Panel")
        
        col1, col2, col3 = st.columns(3)
        col1.metric("Emissions Avoided", f"{environmental['co2_avoided']:,.0f} tons CO₂",
                   help=f"Equivalent to {environmental['trees_equivalent']:,} trees")
        col2.metric("Farmer Income", f"₹{economics['cost']['farmer_payment']/100000:.2f}L",
                   delta=f"₹{economics['cost']['farmer_payment']/len(df):,.0f} per farmer")
        col3.metric("Coal Replaced", f"{environmental['coal_replaced']:,.0f} tons",
                   help=f"Powers ~{environmental['homes_powered']} homes/year")
        
        st.markdown("#### Farmer Income Impact")
        st.caption("Direct benefit to farmers vs traditional burning (₹0 income)")
        
        farmer_income_df = df.copy()
        farmer_income_df['Income (INR)'] = farmer_income_df['Stubble (tons)'] * assumptions['farmer_payment_per_ton']
        farmer_income_df = farmer_income_df.sort_values('Income (INR)', ascending=False).head(10)
        
        fig_income = px.bar(farmer_income_df, x='Farmer Name', y='Income (INR)',
                           title='Top 10 Farmers by Income',
                           color='Income (INR)', color_continuous_scale='Greens')
        st.plotly_chart(fig_income, use_container_width=True)
        ...
        with col1:
            st.markdown("""
            <div class="glass-alert-success">
                <strong>🌍 Environmental Wins:</strong>
                <ul style="margin-top: 0.5rem;">
                    <li>🌳 """ + f"{environmental['trees_equivalent']:,}" + """ trees equivalent</li>
                    <li>🚗 """ + f"{environmental['cars_equivalent']:,}" + """ cars off road for 1 year</li>
                    <li>🏠 """ + f"{environmental['homes_powered']:,}" + """ homes powered</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            <div class="glass-alert-info">
                ...
                <ul style="margin-top: 0.5rem;">
                    <li>👨‍🌾 """ + f"{len(df)}" + """ farmers earning income</li>
                    <li>🏘️ """ + f"{df['Village'].nunique()}" + """ villages benefiting</li>
                    <li>💼 """ + f"{len(cluster_metrics) * 5}" + """ jobs created</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col1:
            ...
            st.markdown(f"""
            - ✅ **{total_stubble:.0f}t** stubble processed
            - ✅ **₹{economics['metrics']['profit']/100000:.2f}L** profit
            - ✅ **{economics['metrics']['margin']:.1f}%** margin
            - ✅ **{logistics['reduction_pct']:.0f}%** logistics saved
            - ✅ **{environmental['co2_avoided']:.0f}t** CO₂ avoided
            """)
        
        with col2:
            pilot_summary = pd.DataFrame({
                'Metric': ['Stubble', 'Revenue', 'Cost', 'Profit', 'Farmers', 'CO₂ Avoided'],
                'Value': [
                    f"{total_stubble:.0f} tons",
                    f"₹{economics['revenue']['total']:,.0f}",
                    f"₹{economics['cost']['total']:,.0f}",
            ...
            st.download_button("📑 Pilot Summary", csv_summary, "pilot_summary.csv", "text/csv")
            
            farmer_csv = df.to_csv(index=False)
            st.download_button("📄 Farmer Data", farmer_csv, "farmer_data.csv", "text/csv")
        
        with col3:
            st.markdown("### 📊 Assumptions")
            st.markdown(f"""
            **{st.session_state.scenario} Scenario**
            - Briquette yield: {assumptions['briquette_yield']*100:.0f}%
            - Biochar yield: {assumptions['biochar_yield']*100:.0f}%
            - Processing: ₹{assumptions['processing_cost_per_ton']}/ton
            - Farmer payment: ₹{assumptions['farmer_payment_per_ton']}/ton
            """)
